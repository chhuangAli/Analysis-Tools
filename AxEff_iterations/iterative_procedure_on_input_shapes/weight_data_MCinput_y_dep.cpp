...
//
  ...
  for(int iCent = 0; iCent < ELEMENT_FOR_CENT_BIN-1; iCent++)
  {
    ...
    tf1_fill_random_jpsi_yield = new TH1F(Form("histoYield_tf1_cent%ito%i", cent_bin[iCent], cent_bin[iCent+1] ),"", number_y_bin-1, y_bin);
    tf1_fill_random_jpsi_yield->Sumw2();

    ratio_jpsi_yield_tf1 = new TH1F(Form("histo_ratio_Yield_tf1_cent%ito%i", cent_bin[iCent], cent_bin[iCent+1] ),"", number_y_bin-1, y_bin);
...
//
  ...
  for(int iCent = 0; iCent < ELEMENT_FOR_CENT_BIN-1; iCent++)
  {
    ...
    tf1_fill_random = new TH1F(Form("histoMCinput_tf1_cent%ito%i", cent_bin[iCent], cent_bin[iCent+1] ),"", number_y_bin-1, y_bin);
    tf1_fill_random->Sumw2();

    ratio_MCinput_tf1 = new TH1F(Form("histo_ratio_MCinput_tf1_cent%ito%i", cent_bin[iCent], cent_bin[iCent+1] ),"", number_y_bin-1, y_bin);
...
//    c2->SetLogy();
    ...
    TF1 *tf_Nraw_y = new TF1("tf_Nraw_y", y_shape, y_bin[0], y_bin[number_y_bin-1], 3);
...
//    t1->AddText(0.,0.,Form("n2:%.4f #pm %.4f", par4, par4_err)  );
    ...
    ((TH1F*) outputFile->Get(Form("RawYieldAndFit/histoYield_tf1_cent%ito%i", cent_bin[iCent], cent_bin[iCent+1]) ))->FillRandom("tf_Nraw_y", 1e7);
    ...
    for(int iCent = 0; iCent < ELEMENT_FOR_CENT_BIN-1; iCent++)
    {
      for (int iY = 0; iY < number_y_bin+1; iY++)
      {
        double yval=0;
        double yval_err = 0;
        yval = ((TH1F*) outputFile->Get(Form("RawYieldAndFit/histoYield_tf1_cent%ito%i", cent_bin[iCent], cent_bin[iCent+1]) ))->GetBinContent(iY+1);
        yval = yval/(y_bin[iY+1]-y_bin[iY]);
        yval_err = ((TH1F*) outputFile->Get(Form("RawYieldAndFit/histoYield_tf1_cent%ito%i", cent_bin[iCent], cent_bin[iCent+1]) ))->GetBinError(iY+1);
        yval_err = yval*( yval_err/(((TH1F*) outputFile->Get(Form("RawYieldAndFit/histoYield_tf1_cent%ito%i", cent_bin[iCent], cent_bin[iCent+1]) ))->GetBinContent(iY+1)));

        ((TH1F*) outputFile->Get(Form("RawYieldAndFit/histoYield_tf1_cent%ito%i", cent_bin[iCent], cent_bin[iCent+1]) ))->SetBinContent(iY+1, yval);
        ((TH1F*) outputFile->Get(Form("RawYieldAndFit/histoYield_tf1_cent%ito%i", cent_bin[iCent], cent_bin[iCent+1]) ))->SetBinError(iY+1, yval_err);
     ...
     for(int iCent = 0; iCent < ELEMENT_FOR_CENT_BIN-1; iCent++)
     {
       ...
       for (int iY = 0; iY < number_y_bin+1; iY++)
       {
           sum_points1 += ((TH1F*) outputFile->Get(Form("RawYieldAndFit/histoYield_tf1_cent%ito%i", cent_bin[iCent], cent_bin[iCent+1]) ))->GetBinContent(iY+1);
           sum_points2 += ((TH1F*) outputFile->Get(Form("RawYieldAndFit/histoYield_cent%ito%i", cent_bin[iCent], cent_bin[iCent+1]) ))->GetBinContent(iY+1);
       }
...
//    c3->SetLogy();
    ...
    TF1 *tf_Ngen_y = new TF1("tf_Ngen_y", y_shape, y_bin[0], y_bin[number_y_bin-1], 3);
...
//    tf_Ngen_pt->FixParameter(3, 3.96363);
    ...
    tf_Ngen_y->SetParLimits(1, -5, 5);  
    tf_Ngen_y->SetParLimits(2, -10, 10);
...
//    t1_MCinput->AddText(0.,0.,Form("n2:%.4f #pm %.4f", par4, par4_err)  );
    ...
    ((TH1F*) outputFile->Get(Form("MCinputAndFit/histoMCinput_tf1_cent%ito%i", cent_bin[iCent], cent_bin[iCent+1]) ))->FillRandom("tf_Ngen_y",1e7);
    ...
    for(int iCent = 0; iCent < ELEMENT_FOR_CENT_BIN-1; iCent++)
    {
      for (int iY = 0; iY < number_y_bin+1; iY++)
      {
        double yval=0;
        double yval_err = 0;
        yval = ((TH1F*) outputFile->Get(Form("MCinputAndFit/histoMCinput_tf1_cent%ito%i", cent_bin[iCent], cent_bin[iCent+1]) ))->GetBinContent(iY+1);
        yval = yval/(y_bin[iY+1]-y_bin[iY]);
        yval_err = ((TH1F*) outputFile->Get(Form("MCinputAndFit/histoMCinput_tf1_cent%ito%i", cent_bin[iCent], cent_bin[iCent+1]) ))->GetBinError(iY+1);
        yval_err = yval*( yval_err/(((TH1F*) outputFile->Get(Form("MCinputAndFit/histoMCinput_tf1_cent%ito%i", cent_bin[iCent], cent_bin[iCent+1]) ))->GetBinContent(iY+1)));

        ((TH1F*) outputFile->Get(Form("MCinputAndFit/histoMCinput_tf1_cent%ito%i", cent_bin[iCent], cent_bin[iCent+1]) ))->SetBinContent(iY+1, yval);
        ((TH1F*) outputFile->Get(Form("MCinputAndFit/histoMCinput_tf1_cent%ito%i", cent_bin[iCent], cent_bin[iCent+1]) ))->SetBinError(iY+1, yval_err);
     ...
     for(int iCent = 0; iCent < ELEMENT_FOR_CENT_BIN-1; iCent++)
     {
       ...
       for (int iY = 0; iY < number_y_bin+1; iY++)
       {
           sum_points1 += ((TH1F*) outputFile->Get(Form("MCinputAndFit/histoMCinput_tf1_cent%ito%i", cent_bin[iCent], cent_bin[iCent+1]) ))->GetBinContent(iY+1);
           sum_points2 += ((TH1F*) outputFile->Get(Form("MCinputAndFit/histoMCinput_cent%ito%i", cent_bin[iCent], cent_bin[iCent+1]) ))->GetBinContent(iY+1);
       }
