...
//  const int FITRANGE = 2;
      ...
      int NUMOFBACKGROUNDFUNC=2;

      // 13 -> pT=12-15
      int pt_bin = 4; // indicate which pT bin to be used. It is only effect when fit the signal as a function of pT
      int y_bin = 2;

      TString tstr_pt_bin = Form("12 < #it{p_{T} < 20"); //for tlatex on mass plot
      TString tstr_cent_bin = Form("centrality 10-20%%");
      TString tstr_system = Form("Pb-Pb, #sqrt{#it{s}_{NN}} = 5.02 TeV");
      TString tstr_y_bin = Form("3.5 < #it{y} < 4"); 
...
//      double display_NJPsi_range = 2000;
      ...
      const int min=0;
  //max is equal to 20 (20:direct fit)
