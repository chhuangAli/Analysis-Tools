...
//      anaResult_file[iRun] = Form("$HOME/local/anaedPbPbData2018/LHC18q_r/muon_calo_pass2/AOD211_CMULEvent_Results/AnalysisResults_run000%i.root",LHC18q_r_run_number[iRun]);
     anaResult_file[iRun] = Form("$HOME/local/anaedPbPbData2018/LHC18q_r/muon_calo_pass3/AOD225_CMULEvent_AnaResults/2018PbPb_CMULResultsHistos_Run000%i.root",LHC18q_r_run_number[iRun]);
...
//  for(int iRun=0; iRun < LHC15o_NUMOFFILES; iRun++)  LHC15o_anaResult_file[iRun] = Form("$HOME/local/anaedPbPbData2015/CMULEvent_Results/AnalysisResults_run000%i.root", LHC15o_run_number[iRun]);
  ...
  double **LHC15o_numOfRejecEvents_Cent_per_Run = new double*[LHC15o_NUMOFFILES];
  for(int iRun=0; iRun< LHC15o_NUMOFFILES; iRun++) LHC15o_numOfRejecEvents_Cent_per_Run[iRun] = new double[MAXINDEX_CENTBINS];
...
//
  ...
  TString counter_name = Form("eventCounters");
...
//  for(int iRun=0; iRun<NUMOFFILES; iRun++)  for(int iCent=0; iCent<MAXINDEX_CENTBINS; iCent++) std::cout << numOfevents_Cent_per_Run[iRun][iCent] << std::endl;
  ...
  for(int iRun=0; iRun<LHC15o_NUMOFFILES; iRun++)
  {
    ...
    {
        LHC15o_numOfevents_per_Run[iRun] += LHC15o_numOfevents_Cent_per_Run[iRun][iCent]; 
//        numOfRejecEvents_per_Run[iRun] += numOfRejecEvents_Cent_per_Run[iRun][iCent];
  ...
  int pTNBins = 200; float pTMinBin = 0; float pTMaxBin =20;
...
//  float Pt_bin_for_Ae[ELEMENT_FOR_AE_PT_BIN]= {0, 0.3, 1, 2, 3, 4, 5, 6, 8, 10, 12, 15, 20};
  ...
  TH1F* hAe_Pt = new TH1F(Form("hAe_Pt"),Form("Ae as function of Pt %s; Pt(GeV/c); A#epsilon", centrality_total_range.Data()), (ELEMENT_FOR_AE_PT_BIN-1), Pt_bin_for_Ae);
...
//  for(int iCent=0; iCent<CENTRALITY_CLASS_SELECTION; iCent++)
    ...
    for(int iRun = 0; iRun < NUMOFFILES_LHC15o_18q_18r; iRun++)
    {
        ...
        {
          inputSimFile = Form("$HOME/local/anaedPbPbSimData2016/LHC16e2_with_centrality_class/AnalysisResults_run000%i_MCembedding_LHC16e2.root", LHC15o_run_number[iRun]);
          inputSimFile2 = Form("$HOME/local/anaedPbPbSimData2016/LHC16e2_plus_with_centrality_class/AnalysisResults_run000%i_MCembedding_LHC16e2_plus.root", LHC15o_run_number[iRun]);
...
//          std::cout << LHC18q_r_run_number[iRun-LHC15o_NUMOFFILES] << std::endl;
          inputSimFile = Form("$HOME/local/anaedPbPbSim2018/LHC19a2/AnalysisResults_run%i.root",LHC18q_r_run_number[iRun-LHC15o_NUMOFFILES]);
          inputFile = TFile::Open( inputSimFile.Data() );        
        }
        else 
        {
...
//                  tlist_gen[1] = (TList *)inputFile_LHC16e2_plus->Get("GeneratedINT7inMUON");
                  ...
                  hPt_gen[iCent][iRun]->Add( (TH1F*)tlist_gen[0]->FindObject( Form("hPt_Gen%s", centrality_range.Data())) );
                  if(iRun < LHC15o_NUMOFFILES) hPt_gen[iCent][iRun]->Add( (TH1F*)tlist_gen[1]->FindObject( Form("hPt_Gen%s", centrality_range.Data())) );
...
//
  ...
  for(int iCent=0; iCent<(MAX_CENTRALITY_CLASS_SELECTION-2); iCent++)  
  {
      for(int iRun=0; iRun<NUMOFFILES_LHC15o_18q_18r; iRun++)  
      {
        if(iRun < LHC15o_NUMOFFILES) 
        {
           Ae_per_run[iRun] += Ae_per_run_cent[iCent][iRun] * LHC15o_numOfevents_Cent_per_Run[iRun][iCent];
           errorAe_per_run[iRun] += errorAe_per_run_cent[iCent][iRun] * errorAe_per_run_cent[iCent][iRun]* LHC15o_numOfevents_Cent_per_Run[iRun][iCent]*LHC15o_numOfevents_Cent_per_Run[iRun][iCent];
           centrality_events[iRun] += LHC15o_numOfevents_Cent_per_Run[iRun][iCent];
        }
        else 
        {
...
//             std::cout << strPtBins.Data() <<std::endl;             
             ...
             int lower_limit_bin_to_be_integral = ((display_pTMinBin - pTMinBin) / value_each_pTbin )+ 1;
             int upper_limit_bin_to_be_integral = ((display_pTMaxBin - pTMinBin) / value_each_pTbin) + 1;
//             int lower_limit_bin_to_be_integral = hPt_rec[iCent][iRun]->FindBin(display_pTMinBin);
//             int upper_limit_bin_to_be_integral = hPt_rec[iCent][iRun]->FindBin(display_pTMaxBin);
//             std::cout <<"bin number: " << lower_limit_bin_to_be_integral << " " << upper_limit_bin_to_be_integral << std::endl;         
             ...
             double error_phy_for_integral = 0;  double error_MC_for_integral = 0; double error_for_integral=0;
             double entries_hPt_rec = hPt_rec[iCent][iRun]->IntegralAndError(lower_limit_bin_to_be_integral, upper_limit_bin_to_be_integral, error_phy_for_integral);
             double entries_hPt_gen = hPt_gen[iCent][iRun]->IntegralAndError(lower_limit_bin_to_be_integral, upper_limit_bin_to_be_integral, error_MC_for_integral); 
             ...
             entries_hPt_rec_cent_pt[iCent][iPt] += entries_hPt_rec;
...
//             Ae_summation_numerator[iCent] = Ae_summation_numerator[iCent] / total_effective_events[iCent];           
               mean_Ae_run_by_run[iCent] =  Ae_summation_numerator / total_effective_events;
               mean_errorAe_run_by_run[iCent] = sqrt(errorAe_summation_numerator / (total_effective_events*total_effective_events) );
...
//              mean_errorAe_cent_weight += mean_errorAe_run_by_run[iCent]*numOfevents_CentBins[iCent];
        ...
        std::cout <<"Ae: " << std::fixed << std::setprecision(3) <<  mean_Ae_add_cent_weight / total_effective_centrality_events <<" +/- "<< std::fixed << std::setprecision(6) << sqrt( mean_errorAe_cent_weight / (total_effective_centrality_events*total_effective_centrality_events) ) << std::endl;

        hAe_Pt->SetBinContent((iPt+1), mean_Ae_add_cent_weight / total_effective_centrality_events);
        hAe_Pt->SetBinError((iPt+1), sqrt(mean_errorAe_cent_weight / (total_effective_centrality_events*total_effective_centrality_events) ));
