...
//  const int CENTRALITY_CLASS_SELECTION = 2; // for 0-10% and 10-20%
   const int CENTRALITY_CLASS_SELECTION = 5; // for 0-10%, 10-20%, 20-30%, 30-40% and 40-50%
  ...
  TH1F *hPt_ratio[MAX_CENTRALITY_CLASS_SELECTION][NUMOFFILES_LHC15o_18q_18r];
  
  TH1F *hPt_rec_sum = new TH1F("hPt_rec_sum",";Pt(GeV/c); # of events", pTNBins,pTMinBin,pTMaxBin);
  TH1F *hPt_gen_sum = new TH1F("hPt_gen_sum",Form("%s;Pt(GeV/c); # of events", centrality_total_range.Data()), pTNBins,pTMinBin,pTMaxBin);

  TH1F *hPt_gen_minus_rec[MAX_CENTRALITY_CLASS_SELECTION];
  TH1F *hPt_gen_minus_rec_with_small_rec[MAX_CENTRALITY_CLASS_SELECTION];
...
//  float Pt_bin_for_Ae[ELEMENT_FOR_AE_PT_BIN]= {0, 0.3, 1, 2, 3, 4, 5, 6, 8, 10, 12, 15, 20};
  ...
  TH1F *hM_rec[MAX_CENTRALITY_CLASS_SELECTION][ELEMENT_FOR_AE_PT_BIN][NUMOFFILES_LHC15o_18q_18r];
  TH1F *hM_gen[MAX_CENTRALITY_CLASS_SELECTION][ELEMENT_FOR_AE_PT_BIN][NUMOFFILES_LHC15o_18q_18r];
  ...
  {
      hAe_Pt_cent_dep[iCent] = new TH1F(Form("hAe_Pt_cent%ito%i", arrayTuningCentralityBins[iCent], arrayTuningCentralityBins[iCent+1]),Form("Ae as function of Pt cent%ito%i; Pt(GeV/c); A#epsilon", arrayTuningCentralityBins[iCent], arrayTuningCentralityBins[iCent+1]), (ELEMENT_FOR_AE_PT_BIN-1), Pt_bin_for_Ae);     
      hAe_Pt_cent_dep[iCent]->Sumw2();
  }
  
  TH1F* hAe_Pt_merged_cent[2];
  hAe_Pt_merged_cent[0] = new TH1F(Form("hAe_Pt_cent%ito%i", arrayTuningCentralityBins[2], arrayTuningCentralityBins[4]),Form("Ae as function of Pt cent%ito%i; Pt(GeV/c); A#epsilon", arrayTuningCentralityBins[2], arrayTuningCentralityBins[4]), (ELEMENT_FOR_AE_PT_BIN-1), Pt_bin_for_Ae);
  hAe_Pt_merged_cent[0]->Sumw2();

  hAe_Pt_merged_cent[1] = new TH1F(Form("hAe_Pt_cent%ito%i", arrayTuningCentralityBins[3], arrayTuningCentralityBins[5]),Form("Ae as function of Pt cent%ito%i; Pt(GeV/c); A#epsilon", arrayTuningCentralityBins[3], arrayTuningCentralityBins[5]), (ELEMENT_FOR_AE_PT_BIN-1), Pt_bin_for_Ae);
...
//  for(int iCent=0; iCent<CENTRALITY_CLASS_SELECTION; iCent++)
    ...
    for(int iCent=0; iCent<(MAX_CENTRALITY_CLASS_SELECTION-1); iCent++ )
    {
          ...
          for(int iRun=0; iRun < NUMOFFILES_LHC15o_18q_18r; iRun++)
          {
                  ...
                  hPt_ratio[iCent][iRun] = new TH1F(Form("hPt_ratio_%i_%i", iCent, iRun),"hPt ratio; p_{T}(GeV/c); ratio: dN/dp_{T}^{rec} / p_{T}^{MC}",pTNBins,pTMinBin,pTMaxBin);
                  hPt_ratio[iCent][iRun]->Sumw2();
          }
...
//  for(int iCent=0; iCent<CENTRALITY_CLASS_SELECTION; iCent++)
    ...
    TString output_filename = "./AxEff_LHC15o_18q_18r.root";
//    TString output_filename = "./AxEff_LHC15o_18q_18r_1st_tuning.root";
    ...
    for(int iRun = 0; iRun < NUMOFFILES_LHC15o_18q_18r; iRun++)
    {
        ...
        {
          inputSimFile = Form("$HOME/local/anaedPbPbSimData2016/LHC16e2_jpsi_ttree/CINTEvent_AnaResults_check/PbPbMC_ResultsHistos_Run%i.root", LHC15o_run_number[iRun]);
          inputSimFile2 = Form("$HOME/local/anaedPbPbSimData2016/LHC16e2_plus_jpsi_ttree/CINTEvent_AnaResults_check/PbPbMC_ResultsHistos_Run%i.root", LHC15o_run_number[iRun]);

//          inputSimFile = Form("$HOME/local/anaedPbPbSimData2016/LHC16e2_jpsi_ttree/CINTEvent_AnaResults/PbPbMC_ResultsHistos_Run%i.root", LHC15o_run_number[iRun]);
//          inputSimFile2 = Form("$HOME/local/anaedPbPbSimData2016/LHC16e2_plus_jpsi_ttree/CINTEvent_AnaResults/PbPbMC_ResultsHistos_Run%i.root", LHC15o_run_number[iRun]);
...
//          std::cout << LHC18q_r_run_number[iRun-LHC15o_NUMOFFILES] << std::endl;
          inputSimFile = Form("$HOME/local/anaedPbPbSim2018/LHC19a2_jpsi_ttree/CINTEvent_AnaResults_check/PbPbMC_ResultsHistos_Run%i.root",LHC18q_r_run_number[iRun-LHC15o_NUMOFFILES]);
...
//                    std::cout << pTrange.Data() << std::endl;
                    ...
                    hPt_gen_sum->Add(hPt_gen[iCent][iRun]);
...
//                  delete tlist_gen[1]; 
  ...
  c1->SetLogy();
  hPt_gen_sum->SetMarkerColor(2);
  hPt_gen_sum->SetLineColor(2);
  hPt_gen_sum->Draw();
  hPt_rec_sum->Draw("SAME");
  c1->Update();
 
  double error;
  std::cout << "Gen Njpsi: " << hPt_gen_sum->Integral(1, 200) << " +/- " << error << std::endl;
  error=0;
  std::cout << "Rec Njpsi: " << hPt_rec_sum->Integral(1, 200) << " +/- " << error << std::endl;
...
//
//
// Calculate the AxEff(pT = 0-20) in each centrality bin per run
//
//
  double entries_rec = 0;
  double entries_gen = 0;
...
//      for(int iRun=0; iRun<1; iRun++)
      {
        entries_rec_per_run[iCent][iRun] += hPt_rec[iCent][iRun]->Integral(1, pTNBins); // pT = 0-20 GeV/c
        entries_gen_per_run[iCent][iRun] += hPt_gen[iCent][iRun]->Integral(1, pTNBins); // pT = 0-20 GeV/c
//          entries_rec_per_run[iCent][iRun] += hPt_rec[iCent][iRun]->Integral(1, 120); // pT = 0-12 GeV/c
//          entries_gen_per_run[iCent][iRun] += hPt_gen[iCent][iRun]->Integral(1, 120); // pT = 0-12 GeV/c
//        entries_rec += hPt_rec[iCent][iRun]->Integral(1, pTNBins);
//        entries_gen += hPt_gen[iCent][iRun]->Integral(1, pTNBins);

        for(int ipT=0; ipT< (ELEMENT_FOR_AE_PT_BIN-1); ipT++)
        {
          entries_rec += hM_rec[iCent][ipT][iRun]->Integral(1,400);
          entries_gen += hM_gen[iCent][ipT][iRun]->Integral(1,400);
        }
      }
  }
  
  std::cout << "total j/psi: " << std::fixed << std::setprecision(1) << entries_rec << ", " << entries_gen << std::endl;
...
//             std::cout << strPtBins.Data() <<std::endl;             
    ...
    double entries_hPt_gen = hPt_gen_sum->IntegralAndError(lower_limit_bin_to_be_integral,upper_limit_bin_to_be_integral, error_phy_for_integral);
    double entries_hPt_rec = hPt_rec_sum->IntegralAndError(lower_limit_bin_to_be_integral,upper_limit_bin_to_be_integral, error_phy_for_integral);
...
//  double meta_Ae[CENTRALITY_CLASS_SELECTION][NUMOFFILES_LHC15o_18q_18r][numberOfPtBinsPhotonProduction] = {0};
//  double meta_errorAe[CENTRALITY_CLASS_SELECTION][NUMOFFILES_LHC15o_18q_18r][numberOfPtBinsPhotonProduction]={0};
//  double meta_numOfevents_per_Run[CENTRALITY_CLASS_SELECTION][NUMOFFILES_LHC15o_18q_18r][numberOfPtBinsPhotonProduction] ={0};
//  double meta_numOfevents_per_Run[NUMOFFILES_LHC15o_18q_18r][CENTRALITY_CLASS_SELECTION] ={0};
...
//      std::cout << "ipT:" << Pt_bin_for_Ae[iPt] << " " << Pt_bin_for_Ae[iPt+1] <<std::endl;
...
//      for(int iRun = 0; iRun < NUMOFFILES_LHC15o_18q_18r; iRun++)
      for(int iPt=0; iPt<(ELEMENT_FOR_AE_PT_BIN-1); iPt++)
//      for(int iPt=0; iPt<(ELEMENT_FOR_AE_PT_BIN-4); iPt++)
      {

//             if(iRun >= LHC15o_NUMOFFILES) std::cout << LHC18q_r_run_number[iRun-LHC15o_NUMOFFILES] <<std::endl;
             TString strPtBins;
//             strPtBins.Form("Pt%gto%g",arrayPtBinsPhotonProduction[iPt],arrayPtBinsPhotonProduction[iPt+1]);
             strPtBins.Form("Pt%gto%g",Pt_bin_for_Ae[iPt],Pt_bin_for_Ae[iPt+1]);
//             strPtBins.Form("Pt%gto%g",Pt_bin_for_Ae_2015[iPt],Pt_bin_for_Ae_2015[iPt+1]);
//             std::cout << strPtBins.Data() <<std::endl;             
             
             int lower_limit_bin_to_be_integral = hPt_rec[iCent][iRun]->FindBin(Pt_bin_for_Ae[iPt]);
             int upper_limit_bin_to_be_integral = hPt_rec[iCent][iRun]->FindBin(Pt_bin_for_Ae[iPt+1]);

//             int lower_limit_bin_to_be_integral = hPt_rec[iCent][iRun]->FindBin(Pt_bin_for_Ae_2015[iPt]);
 //            int upper_limit_bin_to_be_integral = hPt_rec[iCent][iRun]->FindBin(Pt_bin_for_Ae_2015[iPt+1]);
...
//             std::cout <<"bin number: " << lower_limit_bin_to_be_integral << " " << upper_limit_bin_to_be_integral << std::endl;         
             ...
             //
             for(int iBin=1; iBin<= pTNBins; iBin++)
             {
                    if(hPt_ratio[iCent][iRun]->GetBinContent(iBin) ==0 ) hPt_ratio[iCent][iRun]->SetBinError(iBin,0 );
                    else hPt_ratio[iCent][iRun]->SetBinError(iBin, cal_binomial_error(hPt_rec[iCent][iRun]->GetBinContent(iBin), hPt_gen[iCent][iRun]->GetBinContent(iBin)));
                    //std::cout << hPt_ratio->GetBinContent(iBin) <<"+/-"<< hPt_ratio->GetBinError(iBin) << std::endl;
             }

             double error_phy_for_integral = 0;  double error_MC_for_integral = 0; double error_for_integral=0;

//             entries_hPt_rec = hPt_rec[iCent][iRun]->IntegralAndError(lower_limit_bin_to_be_integral, upper_limit_bin_to_be_integral-1, error_phy_for_integral);
//             entries_hPt_gen = hPt_gen[iCent][iRun]->IntegralAndError(lower_limit_bin_to_be_integral, upper_limit_bin_to_be_integral-1, error_MC_for_integral); 

             entries_hPt_rec = hM_rec[iCent][iPt][iRun]->Integral(1,invMassNBins);
             entries_hPt_gen = hM_gen[iCent][iPt][iRun]->Integral(1,invMassNBins);

//             double entries_hPt_rec += hM_rec[iCent][iPt][iRun]->Integral(1,invMassNBins);
//             double entries_hPt_gen += hM_gen[iCent][iPt][iRun]->Integral(1,invMassNBins);
/*             
             if(entries_hPt_gen < entries_hPt_rec)
             {                
                iCounter += 1;
                if(iRun < LHC15o_NUMOFFILES)
                {
                ...
                else 
                {
                  ...
                  hRun_rec_greater_gen[iCent]->SetBinContent(iCounter,1);
                  std::cout << "run number with #recjpsi > #genjpsi: " << LHC18q_r_run_number[iRun-LHC15o_NUMOFFILES] << std::endl;
                }
             }
*/
             entries_hPt_rec_cent_pt[iCent][iPt] += entries_hPt_rec; // sum over runs
             entries_hPt_gen_cent_pt[iCent][iPt] += entries_hPt_gen;
...
//                       error_phy_for_integral=0; error_MC_for_integral=0;
                       ...
                       if(iRun < LHC15o_NUMOFFILES)          
                       {
//                           meta_numOfevents_per_Run[iCent][iRun][iPt] = LHC15o_numOfevents_Cent_per_Run[iRun][iCent+1];
//                           meta_numOfevents_per_Run[iRun][iCent] = LHC15o_numOfevents_Cent_per_Run[iRun][iCent+1];
...
//                       else  meta_numOfevents_per_Run[iCent][iRun][iPt] = numOfevents_Cent_per_Run[iRun-LHC15o_NUMOFFILES][iCent+1];
...
//                       meta_numOfevents_per_Run[iRun][iCent][iPt] = numOfevents_per_Run[iRun];
                ...
                else if(entries_hPt_gen == 0 )
                {
                  Ae = 0;
                  error_Ae = 0;
                  meta_numOfevents_per_Run[iRun][iCent][iPt] = 0;
//                  meta_numOfevents_per_Run[iCent][iRun][iPt] = 0;
                  if(iRun < LHC15o_NUMOFFILES) 
                  {
                    std::cout << LHC15o_run_number[iRun] << " " << Pt_bin_for_Ae[iPt] << " " << Pt_bin_for_Ae[iPt+1];
                    std::cout << " " << entries_hPt_rec << " " << entries_hPt_gen << std::endl;
                  }
                  else 
                  {
                    std::cout << LHC18q_r_run_number[iRun-LHC15o_NUMOFFILES] << " " << Pt_bin_for_Ae[iPt] << " " << Pt_bin_for_Ae[iPt+1];
                    std::cout << " " << entries_hPt_rec << " " << entries_hPt_gen << std::endl;
                  }
...
//             }
/*
           std::cout <<"Run number: " << LHC18q_r_run_number[iRun] << " ";
             std::cout <<"Ae: "<< Ae;
             std::cout << "+/-"<< error_Ae;
...
//             std::cout << entries_hPt_gen << " " << entries_hPt_rec << std::endl;
//             meta_Ae[iCent][iRun][iPt] = Ae;
 //            meta_errorAe[iCent][iRun][iPt] = error_Ae * error_Ae ;
 //            std::cout << entries_hPt_gen << " " << entries_hPt_rec << std::endl;
...
//      std::cout << std::endl;
  ...
  std::cout <<  " where the bug " << std::endl;
...
//       std::cout << meta_numOfruns[iCent][iPt] - NUMOFFILES << std::endl;
       ...
       std::cout << entries_hPt_rec_cent_pt[iCent][iPt] << " " << entries_hPt_gen_cent_pt[iCent][iPt] << " ";
       std::cout << Ae_cent_pt[iCent][iPt] << std::endl;
    }
    std::cout << std::endl;
  }
...
//                               Ae_summation_numerator += (meta_Ae[iCent][iRun][iPt] * meta_numOfevents_per_Run[iCent][iRun][iPt]);
//                               errorAe_summation_numerator += (meta_errorAe[iCent][iRun][iPt] *meta_numOfevents_per_Run[iCent][iRun][iPt]*meta_numOfevents_per_Run[iCent][iRun][iPt]);
                               ...
                               total_effective_events += meta_numOfevents_per_Run[iRun][iCent][iPt]; //for run number weighting
//                               total_effective_events += meta_numOfevents_per_Run[iCent][iRun][iPt]; //for run number weighting
...
*/
        ...
        std::cout <<strPtBins.Data() << " " << std::fixed << std::setprecision(1) << entries_hPt_rec_cent_pt[iCent][iPt] << " " << entries_hPt_gen_cent_pt[iCent][iPt];
        std::cout <<" " << std::fixed << std::setprecision(5) <<  mean_Ae_run_by_run[iPt] <<" +/- "<< std::fixed << std::setprecision(6) << mean_errorAe_run_by_run[iPt] << std::endl;
//        std::cout <<" " << std::fixed << std::setprecision(5) << Ae_cent_pt[iCent][iPt] <<" +/- "<< std::fixed << std::setprecision(6) << errorAe_cent_pt[iCent][iPt] << std::endl;

//        hAe_Pt->SetBinContent((iPt+1), mean_Ae_run_by_run[iPt]);
//        hAe_Pt->SetBinError((iPt+1), mean_errorAe_run_by_run[iPt] ));

        hAe_Pt_cent_dep[iCent]->SetBinContent((iPt+1), mean_Ae_run_by_run[iPt]);
        hAe_Pt_cent_dep[iCent]->SetBinError((iPt+1), mean_errorAe_run_by_run[iPt] );

//        hAe_Pt_cent_dep[iCent]->SetBinContent((iPt+1), Ae_cent_pt[iCent][iPt]);
//        hAe_Pt_cent_dep[iCent]->SetBinError((iPt+1), errorAe_cent_pt[iCent][iPt] );
...
//        std::cout << " " << relative_diff << std::endl;
    ...
    std::cout << " integral AxEff over pT:  " << (hAe_Pt_cent_dep[iCent]->Integral(1, ELEMENT_FOR_AE_PT_BIN)) << std::endl;
  }
...
//
  ...
  TH1F* hAe_Pt_merged_cent0to20;
  hAe_Pt_merged_cent0to20 = new TH1F(Form("hAe_Pt_cent%ito%i", arrayTuningCentralityBins[0], arrayTuningCentralityBins[2]),Form("Ae as function of Pt cent%ito%i; Pt(GeV/c); A#epsilon", arrayTuningCentralityBins[0], arrayTuningCentralityBins[2]), (ELEMENT_FOR_AE_PT_BIN-1), Pt_bin_for_Ae);
  hAe_Pt_merged_cent0to20->Sumw2();

  for(int iMerge = 0; iMerge < 1; iMerge++)
  {
    int merge_min = 0;
    int merge_max = 2;
    std::cout << "revised region" << std::endl;
    for(int iPt=0; iPt<(ELEMENT_FOR_AE_PT_BIN-1); iPt++)
    {
      Ae_summation_numerator=0;
      errorAe_summation_numerator=0;
      total_effective_events=0;    
      for(int iCent = merge_min; iCent < merge_max; iCent++)
      {
         Ae_summation_numerator += ((hAe_Pt_cent_dep[iCent]->GetBinContent(iPt+1))*entries_hPt_rec_cent_pt[iCent][iPt]);
         errorAe_summation_numerator += (hAe_Pt_cent_dep[iCent]->GetBinError(iPt+1))*(hAe_Pt_cent_dep[iCent]->GetBinError(iPt+1))*entries_hPt_rec_cent_pt[iCent][iPt]*entries_hPt_rec_cent_pt[iCent][iPt];
       
         total_effective_events += entries_hPt_rec_cent_pt[iCent][iPt];
      }    
      hAe_Pt_merged_cent0to20->SetBinContent(iPt+1, Ae_summation_numerator/total_effective_events);
      hAe_Pt_merged_cent0to20->SetBinError(iPt+1, sqrt(errorAe_summation_numerator / (total_effective_events*total_effective_events) ));

      std::cout << hAe_Pt_merged_cent0to20->GetBinContent(iPt+1) << " " << hAe_Pt_merged_cent0to20->GetBinError(iPt+1) << " ";
      std::cout << 100.0*(hAe_Pt_merged_cent0to20->GetBinError(iPt+1)) / (hAe_Pt_merged_cent0to20->GetBinContent(iPt+1)) << std::endl;
    }
    std::cout << "end of revised region" << std::endl;
  }
...
//
  ...
  for(int iMerge = 0; iMerge < 2; iMerge++)
  {
    ...
    for(int iPt=0; iPt<(ELEMENT_FOR_AE_PT_BIN-1); iPt++)
    {
      ...
      hAe_Pt_merged_cent[iMerge]->SetBinContent(iPt+1, Ae_summation_numerator/total_effective_events);
      hAe_Pt_merged_cent[iMerge]->SetBinError(iPt+1, sqrt(errorAe_summation_numerator / (total_effective_events*total_effective_events) ));
      std::cout << hAe_Pt_merged_cent[iMerge]->GetBinContent(iPt+1) << " " << hAe_Pt_merged_cent[iMerge]->GetBinError(iPt+1) << " ";
      std::cout << 100.0*(hAe_Pt_merged_cent[iMerge]->GetBinError(iPt+1)) / (hAe_Pt_merged_cent[iMerge]->GetBinContent(iPt+1)) << std::endl;
    }
  }


  TFile *outputFile = new TFile(Form(output_filename.Data()),"recreate");
  for(int iCent = 0; iCent < (CENTRALITY_TUNING); iCent++)
  {
    outputFile->Add(hAe_Pt_cent_dep[iCent]);
  }
  outputFile->Add(hAe_Pt_merged_cent[0]);
  outputFile->Add(hAe_Pt_merged_cent[1]);
  outputFile->Add(hAe_Pt_merged_cent0to20);
  ...
  t2->AddText(0.,0., "Centrality 0 - 10%" );
