...
//        numOfRejecEvents_per_Run[iRun] += numOfRejecEvents_Cent_per_Run[iRun][iCent];
  ...
  TH1F *hY_rec_[MAX_CENTRALITY_CLASS_SELECTION][NUMOFFILES_LHC15o_18q_18r];
  TH1F *hY_gen_[MAX_CENTRALITY_CLASS_SELECTION][NUMOFFILES_LHC15o_18q_18r];
...
//  float Pt_bin_for_Ae[ELEMENT_FOR_AE_PT_BIN]= {0, 0.3, 1, 2, 3, 4, 5, 6, 8, 10, 12, 15, 20};
  ...
  TH1F* hRec[CENTRALITY_TUNING];
  TH1F* hGen[CENTRALITY_TUNING];
  ...
  for(int iCent = 0; iCent < (CENTRALITY_TUNING); iCent++)
  {
      ...
      hRec[iCent] = new TH1F(Form("hRec_cent%ito%i", arrayTuningCentralityBins[iCent], arrayTuningCentralityBins[iCent+1]),Form(";y; number of rec. J/psi "), (number_y_bin), y_bin);
      hGen[iCent]  = new TH1F(Form("hGen_cent%ito%i", arrayTuningCentralityBins[iCent], arrayTuningCentralityBins[iCent+1]),Form(";y; number of gen. J/psi"), (number_y_bin), y_bin);
  }
...
//          hPt_gen_minus_rec_with_small_rec[iCent]->Sumw2();
          ...
          for(int iRun=0; iRun < NUMOFFILES_LHC15o_18q_18r; iRun++)
          {
                  ...
                  hY_rec_[iCent][iRun] = new TH1F(Form("hY__phy_%i_%i", iCent, iRun),"hY_phy", number_y_bin,  y_bin);
                  hY_rec_[iCent][iRun]->SetStats(kTRUE);
                  hY_rec_[iCent][iRun]->GetXaxis()->SetTitle("y");
                  hY_rec_[iCent][iRun]->Sumw2();

                  hY_gen_[iCent][iRun] = new TH1F(Form("hY__Gen_%i_%i", iCent, iRun ),"hY_Gen", number_y_bin,  y_bin);
                  hY_gen_[iCent][iRun]->SetStats(kTRUE);
                  hY_gen_[iCent][iRun]->GetXaxis()->SetTitle("y") ;
                  hY_gen_[iCent][iRun]->Sumw2();
...
//          inputSimFile = Form("$HOME/local/anaedPbPbSim2018/LHC19a2_jpsi_ttree/CINTEvent_AnaResults/PbPbMC_ResultsHistos_Run%i.root",LHC18q_r_run_number[iRun-LHC15o_NUMOFFILES]);
          inputFile = TFile::Open( inputSimFile.Data() );        
        }
        else 
        {
...
//                  tlist_gen[1] = (TList *)inputFile_LHC16e2_plus->Get("GeneratedINT7inMUON");
                  ...
                  hY_rec_[iCent][iRun]->Add( (TH1F*)inputFile->Get( Form("DimuonHistosRecJpsi/RecJpsi_Y%s", centrality_range.Data())) );
                  if(iRun < LHC15o_NUMOFFILES) hY_rec_[iCent][iRun]->Add( (TH1F*)inputFile2->Get( Form("DimuonHistosRecJpsi/RecJpsi_Y%s", centrality_range.Data() )) );

                  hY_gen_[iCent][iRun]->Add( (TH1F*)inputFile->Get( Form("DimuonHistosGenJpsi/GenJpsi_Y%s", centrality_range.Data())) );
                  if(iRun < LHC15o_NUMOFFILES) hY_gen_[iCent][iRun]->Add( (TH1F*)inputFile2->Get( Form("DimuonHistosGenJpsi/GenJpsi_Y%s", centrality_range.Data())) );
...
//           entries_gen_hM += hM_gen[iCent][iY][iRun]->GetEntries();
  ...
  std::cout << "total " <<  std::fixed << std::setprecision(1) << entries_rec_hM << " " << entries_gen_hM << std::endl;
...
//        entries_rec_hM += hY_rec_[iCent][iRun]->GetBinContent(3);
//        entries_gen_hM += hY_gen_[iCent][iRun]->GetBinContent(3);
      }
  }
  
  std::cout <<"y__:" << entries_rec_hM << " " << entries_gen_hM << std::endl;
...
//
  ...
  double entries_hPt_rec2_cent_pt[CENTRALITY_CLASS_SELECTION][ELEMENT_FOR_AE_PT_BIN] = {0};
  double entries_hPt_gen2_cent_pt[CENTRALITY_CLASS_SELECTION][ELEMENT_FOR_AE_PT_BIN] = {0};
...
//      std::cout << "Y: " << y_bin[iPt] << " " << y_bin[iPt+1] << std::endl;
//      for(int iPt=0; iPt<7; iPt++)
//      for(int iPt=0; iPt<(number_y_bin); iPt++)

//      double entries_hY_rec = 0;
//      double entries_hY_gen = 0;

//      double entries_hY_rec2 = 0;
//      double entries_hY_gen2 = 0;

//      for(int iRun = 0; iRun < NUMOFFILES_LHC15o_18q_18r; iRun++)
      for(int iPt=0; iPt<(number_y_bin); iPt++)
//      for(int iPt=(number_y_bin-1); iPt< (number_y_bin); iPt++)
      {
              double entries_hY_rec = 0;
              double entries_hY_gen = 0;
...
//             std::cout <<"bin number: " << lower_limit_bin_to_be_integral << " " << upper_limit_bin_to_be_integral << std::endl;         
             ...
             entries_hY_rec2 = hY_rec[iCent][iRun]->IntegralAndError(lower_limit_bin_to_be_integral, upper_limit_bin_to_be_integral-1, error_phy_for_integral);
             entries_hY_gen2 = hY_gen[iCent][iRun]->IntegralAndError(lower_limit_bin_to_be_integral, upper_limit_bin_to_be_integral-1, error_MC_for_integral); 


//             entries_hY_rec = hY_rec[iCent][iRun]->IntegralAndError(lower_limit_bin_to_be_integral, upper_limit_bin_to_be_integral-1, error_phy_for_integral);
//             entries_hY_gen = hY_gen[iCent][iRun]->IntegralAndError(lower_limit_bin_to_be_integral, upper_limit_bin_to_be_integral-1, error_MC_for_integral);

             entries_hY_rec = hM_rec[iCent][5-iPt][iRun]->Integral(1,400);
             entries_hY_gen = hM_gen[iCent][5-iPt][iRun]->Integral(1,400);
/*
             if( entries_hY_gen!=entries_hY_gen2)
             { 
               std::cout << "For Ngen, at iCent: " << iCent << std::endl;
               if(iRun < LHC15o_NUMOFFILES)
               { 
                 std::cout << " at " << strYBins.Data() << " in run:" << LHC15o_run_number[iRun] << std::endl;
                 std::cout << (lower_limit_bin_to_be_integral-0) << ", " << (upper_limit_bin_to_be_integral-1) << std::endl;
                 std::cout << " from mass dis: " << entries_hY_gen << "  from y dis:" << entries_hY_gen2 << std::endl;
               }
               else
               {
                  std::cout << " at " << strYBins.Data() << " in run:" << LHC18q_r_run_number[iRun-LHC15o_NUMOFFILES] << std::endl;
                  std::cout << (lower_limit_bin_to_be_integral-0) << ", " << (upper_limit_bin_to_be_integral-1) << std::endl;
                  std::cout << " from mass dis: " << entries_hY_gen << "  from y dis:" << entries_hY_gen2 << std::endl;
               }
             }
             ...
             if( entries_hY_rec!=entries_hY_rec2)
             {               
               std::cout << "For Nrec at iCent: " << iCent << std::endl;
               if(iRun < LHC15o_NUMOFFILES)
               {                 
                 std::cout << " at " << strYBins.Data() << " in run:" << LHC15o_run_number[iRun] << std::endl;
                 std::cout << (lower_limit_bin_to_be_integral-0) << ", " << (upper_limit_bin_to_be_integral-1) << std::endl;
                 std::cout << " from mass dis: " << entries_hY_rec << "  from y dis:" << entries_hY_rec2 << std::endl;
               }
               else 
               {
                  std::cout << " at " << strYBins.Data() << " in run:" << LHC18q_r_run_number[iRun-LHC15o_NUMOFFILES] << std::endl;
...
*/
//             entries_hY_rec = hM_rec[iCent][5-iPt][iRun]->GetEntries();
//             entries_hY_gen = hM_gen[iCent][5-iPt][iRun]->GetEntries();
             ...
             entries_hPt_rec2_cent_pt[iCent][iPt] += entries_hY_rec2;
             entries_hPt_gen2_cent_pt[iCent][iPt] += entries_hY_gen2;
...
//             std::cout << entries_hY_rec << " " << entries_hY_gen << std::endl;
  ...
  for(int iCent=0; iCent<CENTRALITY_CLASS_SELECTION; iCent++) 
  {
    ...
    for(int iPt=0; iPt< number_y_bin; iPt++) 
    {
       hRec[iCent]->SetBinContent(iPt+1, entries_hPt_rec_cent_pt[iCent][iPt]);
       hGen[iCent]->SetBinContent(iPt+1, entries_hPt_gen_cent_pt[iCent][iPt]);
       hRec[iCent]->SetBinError(iPt+1, sqrt(entries_hPt_rec_cent_pt[iCent][iPt]));
       hGen[iCent]->SetBinError(iPt+1, sqrt(entries_hPt_gen_cent_pt[iCent][iPt]));

       std::cout <<  entries_hPt_rec_cent_pt[iCent][iPt] << " " << entries_hPt_gen_cent_pt[iCent][iPt] << ", " << std::fixed << std::setprecision(4) <<  entries_hPt_rec_cent_pt[iCent][iPt]/ entries_hPt_gen_cent_pt[iCent][iPt] << std::endl;
...
//        (entries_rec_per_run[iCent][iRun]/entries_gen_per_run[iCent][iRun])
//        hAe
...
//        std::cout << " " << relative_diff << std::endl;
    ...
    std::cout << "integral on y: " << (hAe_cent_dep[iCent]->Integral(1, number_y_bin+1)) << std::endl;
  }
...
//  outputFile->Add(histo_Ae_run);
  outputFile->Add(hGen[0]);
  outputFile->Add(hRec[0]);
  ...
  c2->SetLogy();
  hGen[0]->SetAxisRange(100, 1e7, "Y");
  hGen[0]->Draw();
  hRec[0]->Draw("SAME");
  c2->Print("Nrec_Ngen_y.pdf");
