...
//        numOfRejecEvents_per_Run[iRun] += numOfRejecEvents_Cent_per_Run[iRun][iCent];
  ...
  float PtBins_Double_Diff[] = {0.3,2,4,6,12,20};
  const int numberOfPtBins_Double_Diff = 5;

  const int periods = 2;

  TH1F *hY_rec[MAX_CENTRALITY_CLASS_SELECTION][numberOfPtBins_Double_Diff][NUMOFFILES_LHC15o_18q_18r];
  TH1F *hY_gen[MAX_CENTRALITY_CLASS_SELECTION][numberOfPtBins_Double_Diff][NUMOFFILES_LHC15o_18q_18r];
  TH1F *hY_ratio[MAX_CENTRALITY_CLASS_SELECTION][numberOfPtBins_Double_Diff][NUMOFFILES_LHC15o_18q_18r];
...
//          hPt_gen_minus_rec_with_small_rec[iCent]->Sumw2();
      ...
      for(int ipT=0; ipT < numberOfPtBins_Double_Diff; ipT++)
      {
          ...
          {
                  hY_rec[iCent][ipT][iRun] = new TH1F(Form("hY_phy_%i_%i_%i", iCent, ipT, iRun),"hY_phy", numberOfYBins_Double_Diff, y_bin_double_diff);
                  hY_rec[iCent][ipT][iRun]->SetStats(kTRUE);
                  hY_rec[iCent][ipT][iRun]->GetXaxis()->SetTitle("y");
                  hY_rec[iCent][ipT][iRun]->Sumw2();

                  hY_gen[iCent][ipT][iRun] = new TH1F(Form("hY_Gen_%i_%i_%i", iCent, ipT, iRun),"hY_Gen", numberOfYBins_Double_Diff,  y_bin_double_diff);
                  hY_gen[iCent][ipT][iRun]->SetStats(kTRUE);
                  hY_gen[iCent][ipT][iRun]->GetXaxis()->SetTitle("y") ;
                  hY_gen[iCent][ipT][iRun]->Sumw2();

                  hY_ratio[iCent][ipT][iRun] = new TH1F(Form("hY_ratio_%i_%i_%i", iCent, ipT, iRun),"; y; ", numberOfYBins_Double_Diff,  y_bin_double_diff);
...
//                  tlist_gen[1] = (TList *)inputFile_LHC16e2_plus->Get("GeneratedINT7inMUON");
                    ...
                    hY_rec[iCent][ipT][iRun]->Add( (TH1F*)inputFile->Get( Form("DimuonHistosRecJpsi/Rapidity/histoRapidity%s", centrality_range.Data())) );
                    if(iRun < LHC15o_NUMOFFILES) hY_rec[iCent][ipT][iRun]->Add( (TH1F*)inputFile2->Get( Form("DimuonHistosRecJpsi/Rapidity/histoRapidity%s", centrality_range.Data() )) );
                    hY_rec_sum->Add(hY_rec[iCent][ipT][iRun]);

                    hY_gen[iCent][ipT][iRun]->Add( (TH1F*)inputFile->Get( Form("DimuonHistosGenJpsi/Rapidity/histoRapidity%s", centrality_range.Data())) );
                    if(iRun < LHC15o_NUMOFFILES) hY_gen[iCent][ipT][iRun]->Add( (TH1F*)inputFile2->Get( Form("DimuonHistosGenJpsi/Rapidity/histoRapidity%s", centrality_range.Data())) );

                    hY_gen_sum->Add(hY_gen[iCent][ipT][iRun]);

                    hY_ratio[iCent][ipT][iRun]->Divide(hY_rec[iCent][iRun], hY_gen[iCent][iRun]); 
                  }
                  for( int iY=0; iY<number_y_bin; iY++)
                  {
