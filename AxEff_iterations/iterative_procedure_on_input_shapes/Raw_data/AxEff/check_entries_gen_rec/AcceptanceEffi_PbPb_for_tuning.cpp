...
//  for(int iCent=0; iCent<CENTRALITY_CLASS_SELECTION; iCent++)
    ...
    for(int iRun = 0; iRun < NUMOFFILES_LHC15o_18q_18r; iRun++)
    {
        ...
        if(iRun < LHC15o_NUMOFFILES)
        {
          ...
          inputSimFile2 = Form("$HOME/local/anaedPbPbSimData2016/LHC16e2_jpsi_ttree/CINTEvent_AnaResults_check/PbPbMC_ResultsHistos_Run%i.root", LHC15o_run_number[iRun]);
...
//          entries_gen += hM_gen[iCent][ipT][iRun]->Integral(1,400);
  ...
  std::cout << "total j/psi: " << entries_rec << ", " << entries_gen << std::endl;
...
//             std::cout <<"bin number: " << lower_limit_bin_to_be_integral << " " << upper_limit_bin_to_be_integral << std::endl;         
             ...
             double error_phy_for_integral = 0;  double error_MC_for_integral = 0; double error_for_integral=0;
//             double entries_hPt_rec = hPt_rec[iCent][iRun]->IntegralAndError(lower_limit_bin_to_be_integral, upper_limit_bin_to_be_integral-1, error_phy_for_integral);
//             double entries_hPt_gen = hPt_gen[iCent][iRun]->IntegralAndError(lower_limit_bin_to_be_integral, upper_limit_bin_to_be_integral-1, error_MC_for_integral); 
...
//                       meta_numOfevents_per_Run[iRun][iCent][iPt] = numOfevents_per_Run[iRun];
                ...
                else if(entries_hPt_gen == 0 | entries_hPt_rec == 0)
                {
                  std::cout << LHC18q_r_run_number[iRun] << " " << Pt_bin_for_Ae[iPt] << " " << Pt_bin_for_Ae[iPt+1];
...
*/
        ...
        std::cout <<" pT region: " << strPtBins.Data() << " ";
        std::cout <<"Ae: " << std::fixed << std::setprecision(3) <<  mean_Ae_run_by_run[iPt] <<" +/- "<< std::fixed << std::setprecision(6) << mean_errorAe_run_by_run[iPt] << std::endl;
