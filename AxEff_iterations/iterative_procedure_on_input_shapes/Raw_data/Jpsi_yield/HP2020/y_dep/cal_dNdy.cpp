...
//  const int ELEMENT_FOR_CENT_BIN = 3;
  int cent_bin[] = {0,10,20,30,40,90};
//  int cent_bin[] = {20,30,40,90};
  int ELEMENT_FOR_CENT_BIN = sizeof(cent_bin)/sizeof(cent_bin[0]);
...
//  TString ReadFileLocation = Form("$HOME/cernbox/AnaSourceCode/anaJPsi_2015_2018PbPb/AxEff/cent_dep_tuning_input_shapes/Raw_data/AxEff/AxEff_y/AxEff_LHC15o_18q_18r_1st_tuning.root");

//  TString ReadFileLocation = Form("$HOME/cernbox/AnaSourceCode/anaJPsi_2015_2018PbPb/AxEff/AxEff_y_dep/final_AxEff_2015_2018PbPb.root");
  TString ReadFileLocation = Form("$HOME/cernbox/AnaSourceCode/anaJPsi_2015_2018PbPb/AxEff/AxEff_y_dep/final_AxEff_tuned_one_2015_2018PbPb.root");
  TFile *inputFile_AxEff = new TFile(ReadFileLocation,"READ");

  const int CENTRALITY_TUNING = (ELEMENT_FOR_CENT_BIN-1);
  TH1F* hAe_cent_dep[CENTRALITY_TUNING];

  for(int iCent = 0; iCent < (CENTRALITY_TUNING); iCent++)
  {
      hAe_cent_dep[iCent] = new TH1F(Form("hAe_cent%ito%i_", cent_bin[iCent], cent_bin[iCent+1]),Form("Ae as function of y cent%ito%i; y; A#epsilon", cent_bin[iCent], cent_bin[iCent+1]), (number_y_bin-1), y_bin);    
      hAe_cent_dep[iCent]->Sumw2();
  }

  for(int iCent = 0; iCent < (CENTRALITY_TUNING); iCent++)
  {
    hAe_cent_dep[iCent]->Add( (TH1F*)inputFile_AxEff->Get(Form("hAe_Y_cent%ito%i", cent_bin[iCent], cent_bin[iCent+1]) ));
  }

  double Njpsi[2]={0};
  std::ofstream dNdpT_output_stream("dNdy_func_y_PbPb.txt");
  ...
  {        
    directoryJPsi->cd();
    TH1F *jpsi_yields = new TH1F(Form("histoYield_cent%ito%i", cent_bin[iCent], cent_bin[iCent+1] ),"", (number_y_bin-1), y_bin);
    jpsi_yields->Sumw2();   
    directoryJPsi->Add(hAe_cent_dep[iCent]);
  }
...
//  for(int iCent = 2; iCent < 3; iCent++)
  {
    ...
    if(iCent < 2) Njpsi_file_name = "jpsi_number_pp13TeV_VWG_2p2_4p5.txt";
    else Njpsi_file_name = "jpsi_number_CB2_VWG_2p2_4p5.txt";

    for(int ipT=0; ipT < (number_y_bin-1); ipT++)
    {
      TString y_range = Form("y%gto%g/", fabs(y_bin[ipT+1]), fabs(y_bin[ipT]) );   
      std::cout << y_range.Data() << std::endl; 
      TString path_to_Njpsi = Form("%s%s%s%s", path_to_directory.Data(), cent_range.Data(), y_range.Data(), Njpsi_file_name.Data() );
...
//      dNdpT_output_stream << Njpsi[0] << " " << Njpsi[1] << " " << Njpsi[2] << std::endl;
        ...
        xVal = (y_bin[ipT+1] + y_bin[ipT]) / 2;
        xValerr = (y_bin[ipT+1] - y_bin[ipT]) / 2.;
        xValSysterr = 0.3*xValerr;
 
        yVal = Njpsi[0] / (y_bin[ipT+1]-y_bin[ipT]);
        yValerr = yVal * Njpsi[1] / Njpsi[0];

        yValSysterr =  yVal * Njpsi[2] / Njpsi[0];

        std::cout << y_bin[ipT] << " " << y_bin[ipT+1] << " " << (y_bin[ipT+1]-y_bin[ipT]) << std::endl;

        gr_dN_dy->SetPoint(ipT+1, xVal, yVal );
        gr_dN_dy->SetPointError(ipT+1, xValerr, xValerr, yValerr, yValerr);
        ...
        gr_dN_dy_syst->SetPoint(ipT+1, xVal, yVal );
        gr_dN_dy_syst->SetPointError(ipT+1, xValSysterr, xValSysterr, yValSysterr, yValSysterr );

        std::cout << yVal << std::endl;
        dNdpT_output_stream << yVal << " " << gr_dN_dy->GetErrorY(ipT+1) << " " << gr_dN_dy_syst->GetErrorY(ipT+1) <<     std::endl;
    ...
    ((TH1F*) outputFile->Get(Form("JPsiYieldsHistos/histoYield_cent%ito%i", cent_bin[iCent], cent_bin[iCent+1]) ))->Divide(hAe_cent_dep[iCent]);
  } 
