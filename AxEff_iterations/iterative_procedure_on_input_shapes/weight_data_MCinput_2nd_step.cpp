...
//  bool fit_raw = true;
  ...
  if(fit_raw)
  {
    ...
    std::cout << pT_bin[ELEMENT_FOR_AE_PT_BIN-1] << std::endl;
    TF1 *tf_Nraw_pt = new TF1("tf_Nraw_pt", pt_shape, pT_bin[1], pT_bin[ELEMENT_FOR_AE_PT_BIN-1], 4);
    ...
    TF1 *tf_Nraw_pt_fake = new TF1("tf_Nraw_pt", pt_shape, pT_bin[0], pT_bin[ELEMENT_FOR_AE_PT_BIN-1], 4);
...
//
    ...
    double par0_gen_pt_0to10[] = {1474669.55, 3.5027, 1.9340, 3.9636};
    double par0_rec_pt_0to10[] = {1305064.48, 3.1535, 2.3220, 3.2044};

    double par0_gen_pt_10to20[] = {891340.61, 3.5027, 1.9340, 3.9636};
    double par0_rec_pt_10to20[] = {834528.45, 3.7158, 1.9760, 4.1321};
...
/*
    if(iCent == 0)
    {
      ...
      { 
        double Ngen = 0;
        double Ngen_error = 0;
        weight0_pt = setWeight_Pt(pT_bin[ipT], par0_rec_pt_0to10, par0_gen_pt_0to10);
        Ngen = ((TH1F*)inputFile_MCinput->Get(Form("JPsiMCinputHistos/histoMCinput_cent%ito%i", cent_bin[iCent], cent_bin[iCent+1])))->GetBinContent((ipT+1));
        Ngen_error = ((TH1F*)inputFile_MCinput->Get(Form("JPsiMCinputHistos/histoMCinput_cent%ito%i", cent_bin[iCent], cent_bin[iCent+1])))->GetBinError((ipT+1));
        ((TH1F*) outputFile->Get(Form("MCinputAndFit/histoMCinput_cent%ito%i", cent_bin[iCent], cent_bin[iCent+1]) ))->SetBinContent(ipT+1, Ngen*weight0_pt);
        ((TH1F*) outputFile->Get(Form("MCinputAndFit/histoMCinput_cent%ito%i", cent_bin[iCent], cent_bin[iCent+1]) ))->SetBinError(ipT+1, Ngen_error*weight0_pt);
      }
    }
    else if (iCent == 1)
    {
      for (int ipT = 0; ipT < ELEMENT_FOR_AE_PT_BIN; ipT++)
      {
        double Ngen = 0;
        double Ngen_error = 0;
        weight1_pt = setWeight_Pt(pT_bin[ipT], par0_rec_pt_10to20, par0_gen_pt_10to20);
        Ngen = ((TH1F*)inputFile_MCinput->Get(Form("JPsiMCinputHistos/histoMCinput_cent%ito%i", cent_bin[iCent], cent_bin[iCent+1])))->GetBinContent((ipT+1));
        Ngen_error = ((TH1F*)inputFile_MCinput->Get(Form("JPsiMCinputHistos/histoMCinput_cent%ito%i", cent_bin[iCent], cent_bin[iCent+1])))->GetBinError((ipT+1));
//        ((TH1F*) outputFile->Get(Form("MCinputAndFit/histoMCinput_cent%ito%i", cent_bin[iCent], cent_bin[iCent+1]) ))->SetBinContent(ipT+1, Ngen*weight1_pt);
//        ((TH1F*) outputFile->Get(Form("MCinputAndFit/histoMCinput_cent%ito%i", cent_bin[iCent], cent_bin[iCent+1]) ))->SetBinError(ipT+1, Ngen_error*weight1_pt);
      }
    }
*/
...
//    tf_Ngen_pt->SetParameter(3, 3.96363);
    if(iCent == 0)
    {
      tf_Ngen_pt->FixParameter(1, 3.1070);
      tf_Ngen_pt->FixParameter(2, 2.3406);
      tf_Ngen_pt->FixParameter(3, 3.1465);

//      tf_Ngen_pt->SetParameter(1, 3.1535);
//      tf_Ngen_pt->SetParameter(2, 2.3220);
//      tf_Ngen_pt->SetParameter(3, 3.2044);
    }
    else if (iCent == 1)
    {
      tf_Ngen_pt->FixParameter(1, 3.2546);
      tf_Ngen_pt->FixParameter(2, 2.2044);
      tf_Ngen_pt->FixParameter(3, 3.3098);

//      tf_Ngen_pt->SetParameter(1, 3.7158);
//      tf_Ngen_pt->SetParameter(2, 1.9760);
//      tf_Ngen_pt->SetParameter(3, 4.1321);
    }

    tf_Ngen_pt->SetParLimits(0, 100000, 5000000);
//    tf_Ngen_pt->SetParLimits(1, 2, 200);  
//    tf_Ngen_pt->SetParLimits(2, 0, 5);
//    tf_Ngen_pt->SetParLimits(3, 0, 300);
