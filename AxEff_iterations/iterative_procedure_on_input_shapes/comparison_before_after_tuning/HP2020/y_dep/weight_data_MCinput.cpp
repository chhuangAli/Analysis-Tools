...
//  int cent_bin[] = {20,30,40,90};
  ...
  const int ELEMENT_FOR_CENT_BIN = sizeof(cent_bin)/sizeof(cent_bin[0]);  
//
...
//  TString ReadFileLocation = Form("$HOME/cernbox/AnaSourceCode/anaJPsi_2015_2018PbPb/AxEff/cent_dep_tuning_input_shapes/Raw_data/Jpsi_yield/HP2020/y_dep/original_input_shape/JpsiYieldsHistos.root");
  TString ReadFileLocation = Form("$HOME/cernbox/AnaSourceCode/anaJPsi_2015_2018PbPb/AxEff/cent_dep_tuning_input_shapes/Raw_data/Jpsi_yield/HP2020/y_dep/one_tuning/JpsiYieldsHistos.root");
//  TString ReadFileLocation = Form("$HOME/cernbox/AnaSourceCode/anaJPsi_2015_2018PbPb/AxEff/cent_dep_tuning_input_shapes/Raw_data/Jpsi_yield/HP2020/y_dep/one_tuning/JpsiYieldsHistos.root");
...
//  ReadFileLocation = Form("$HOME/cernbox/AnaSourceCode/anaJPsi_2015_2018PbPb/AxEff/cent_dep_tuning_input_shapes/MC_input/HP2020/y_dep/original_input_shape/JpsiMCinputHistos.root");
  ReadFileLocation = Form("$HOME/cernbox/AnaSourceCode/anaJPsi_2015_2018PbPb/AxEff/cent_dep_tuning_input_shapes/MC_input/HP2020/y_dep/one_tuning/JpsiMCinputHistos.root");
...
//
  ...
  TGraphAsymmErrors *gr_dNdpT = new TGraphAsymmErrors( number_y_bin-1 );
  TGraphAsymmErrors *gr_N_jpsi_gen = new TGraphAsymmErrors( number_y_bin-1);
...
//    int iCent = 0;
    ...
    deltaxVal = (y_bin[iPt+1] - y_bin[iPt])/2;
...
//    int iCent = 0;
  ...
  TGraphAsymmErrors *gr_ratio = new TGraphAsymmErrors( number_y_bin-1 );
  for(int iPt = 0; iPt < number_y_bin-1; iPt++)
  {
    double ratio = 0;
//    int iCent = 0;
...
//    gr_ratio-
  ...
  TH2F *histo = new TH2F("histo","",100,-4.5,-2, 100, 0.5e5, 5e7);
//  TH2F *histo = new TH2F("histo","",100,-4.5,-2, 100, 1.0e5, 5e6);
  histo->SetStats(0);
  ...
  histo->GetYaxis()->SetTitle("dN/dy/A#epsilon");

  TH2F *hratio = new TH2F("hratio","",100,-4.5,-2,100, 0.4, 1.3);
  hratio->SetStats(0);
  hratio->GetXaxis()->SetTitleOffset(1.10);
  hratio->GetXaxis()->SetTitle("#it{y} ");
...
//  gr_dNdpT_syst->Draw("2SAME");
 ...
 if(iCent == 0)
  ...
  {
    leg2->SetHeader("20-30\% centrality bin");
  }
  else if(iCent == 3)
  {
    leg2->SetHeader("30-40\% centrality bin");
  }
  else if(iCent == 4)
  {
    leg2->SetHeader("40-90\% centrality bin");
  }
  ...
  leg2->AddEntry(gr_dNdpT, "dN/dy * 1/A#epsilon","LPE");
//  leg2->AddEntry(gr_dNdpT_syst,"Systematic uncertainty","F");
  ...
  TLine *line = new TLine(-4.5,1,-2,1);
  line->SetLineStyle(2);
  line->Draw("same");
  ...
  leg3->AddEntry(gr_ratio, "(dN/dy * 1/A#epsilon) /  input MC (arbitrary norm.)","LPE");
  leg3->Draw();

  if(iCent == 0)
  {
    c2->Print("dN_dy_over_Ae_cent0to10.pdf");
  }
  else if(iCent == 1)
  {
    c2->Print("dN_dy_over_Ae_cent10to20.pdf");
  }
  else if(iCent == 2)
  {
    c2->Print("dN_dy_over_Ae_cent20to30.pdf");
  }
  else if(iCent == 3)
  {
    c2->Print("dN_dy_over_Ae_cent30to40.pdf");
  }
  else if(iCent == 4)
  {
    c2->Print("dN_dy_over_Ae_cent40to90.pdf");
  }
