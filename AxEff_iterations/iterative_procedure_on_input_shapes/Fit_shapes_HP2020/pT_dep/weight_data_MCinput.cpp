...
//
  ...
  TString ReadFileLocation = Form("$HOME/cernbox/AnaSourceCode/anaJPsi_2015_2018PbPb/AxEff/cent_dep_tuning_input_shapes/Raw_data/Jpsi_yield/HP2020/pT_dep/original_input_shape/JpsiYieldsHistos.root");  // before the tuning
//  TString ReadFileLocation = Form("$HOME/cernbox/AnaSourceCode/anaJPsi_2015_2018PbPb/AxEff/cent_dep_tuning_input_shapes/Raw_data/Jpsi_yield/HP2020/pT_dep/one_tuning/JpsiYieldsHistos.root");
...
//
  ...
  for(int iCent = 0; iCent < ELEMENT_FOR_CENT_BIN-1; iCent++)
  {
    ...
    ratio_MCinput_tf1->SetStats(0);
  }

  ReadFileLocation = Form("$HOME/cernbox/AnaSourceCode/anaJPsi_2015_2018PbPb/AxEff/cent_dep_tuning_input_shapes/MC_input/HP2020/pT_dep/original_input_shape/JpsiMCinputHistos.root"); // before the tuning
//  ReadFileLocation = Form("$HOME/cernbox/AnaSourceCode/anaJPsi_2015_2018PbPb/AxEff/cent_dep_tuning_input_shapes/MC_input/HP2020/pT_dep/one_tuning/JpsiMCinputHistos.root"); // first tuning
...
//    tf_Nraw_pt->SetParameter(4, 100);
    tf_Nraw_pt->SetParameter(4, 10000);
//    tf_Nraw_pt->SetParameter(5, 1.0);
    tf_Nraw_pt->SetParameter(5, 0.1);
...
//    tf_Nraw_pt->SetParLimits(4, 0, 1e7);
//    tf_Nraw_pt->SetParLimits(5, 0, 100);
    ...
    TF1 *tf_Nraw_pt_fake = new TF1("tf_Nraw_pt", pt_shape, pT_bin[1], pT_bin[ELEMENT_FOR_AE_PT_BIN-1], 6);
    ...
    for(int iCent = 0; iCent < ELEMENT_FOR_CENT_BIN-1; iCent++)
    {
      for (int ipT = 1; ipT < (ELEMENT_FOR_AE_PT_BIN-1); ipT++)
      {
        double yval=0;
        double yval_err = 0;
        if(ipT == 0)
     ...
     for(int iCent = 0; iCent < ELEMENT_FOR_CENT_BIN-1; iCent++)
     {
       sum_points1 = 0;
       sum_points2 = 0;
       for (int ipT = 1; ipT < ELEMENT_FOR_AE_PT_BIN; ipT++)
       {
...
//    ((TH1F*) outputFile->Get(Form("RawYieldAndFit/histo_ratio_Yield_tf1_cent%ito%i", cent_bin[iCent], cent_bin[iCent+1]) ))->SetBinContent(1,0.);
//    ((TH1F*) outputFile->Get(Form("RawYieldAndFit/histo_ratio_Yield_tf1_cent%ito%i", cent_bin[iCent], cent_bin[iCent+1]) ))->SetBinError(1,0.);
...
//    TF1 *tf_Ngen_pt = new TF1("tf_Ngen_pt", pt_shape, pT_bin[0], pT_bin[ELEMENT_FOR_AE_PT_BIN-1], 4);
     TF1 *tf_Ngen_pt = new TF1("tf_Ngen_pt", pt_shape, pT_bin[1], pT_bin[ELEMENT_FOR_AE_PT_BIN-1], 6);
    if(iStep == 30)
    {
      if(iCent == 0)
      {
    ...
    else 
    {
      ...
      tf_Ngen_pt->SetParameter(4, 100);
//      tf_Ngen_pt->SetParameter(5, 1.5);
      tf_Ngen_pt->SetParameter(5, 0.1);
...
//  tf_Ngen_pt->FixParameter(3, 3.96363);
      ...
      tf_Ngen_pt->SetParLimits(4, -10000, 1.0e10);
//      tf_Ngen_pt->SetParLimits(4, 0, 100000);
//      tf_Ngen_pt->SetParLimits(5, -5, 10);
      tf_Ngen_pt->SetParLimits(5, -10, 100);
    }
